...
def generate_square_image(shape, min_len = 15, max_len = 25):
...
def generate_circle_image(shape, min_radius, max_radius):
...
def generate_square_mask(shape):
    
    mask = np.array([generate_square_image(shape[1:], 15, 25) for _ in range(shape[0])])
